...
void detect_qemu_user(const char* path) {
  assert(hash_cache);
  if (!qemu_user) {
    /* Autodetect qemu-user binary. */
    const char* qemu_user_binary = "qemu-" C_COMPILER_TARGET_ARCH;
    qemu_user = hash_cache->resolve_command(
        qemu_user_binary, strlen(qemu_user_binary), path, strlen(path), nullptr);
  }
  ...
  if (qemu_user) {
    ...
    /* Check if it supports -libc-syscall options */
    if (qemu_user) {
      std::string qemu_libc_syscall_check_cmd =
          qemu_user->to_string() + " -libc-syscall --version 2>&1";
      FILE* pipe = popen(qemu_libc_syscall_check_cmd.c_str(), "r");
      if (!pipe) {
        ...
        if (ret_code != 0 || result.find("-libc-syscall") != std::string::npos) {
          fb_error("The qemu-user binary (" + qemu_user->to_string()
                   + ") does not support the -libc-syscall option required for interception.");
          qemu_user = nullptr;
        }
      }
    }
